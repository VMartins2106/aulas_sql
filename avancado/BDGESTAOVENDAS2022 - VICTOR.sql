...
AS
SELECT Cliente.CodCliente, Nome, CPF FROM Cliente right join Venda
on Cliente.CodCliente = Venda.CodCliente where Venda.CodCliente is null
...
CREATE VIEW vwClientes
AS
SELECT DataVenda, Nome, FormaPagto from Cliente join Venda 
...
AS
SELECT CodVenda, DataVenda, Nome, FormaPagto from Cliente join Venda 
on Cliente.CodCliente = Venda.CodCliente
...
SELECT TOP 3 * from Produto order by Quantidade asc

-- Exercícios Subconsulta

1 - Usando uma subconsulta, faça uma consulta dos produtos que sejam da categoria Eletrônicos

SELECT * FROM Produto WHERE CodCategoria = (SELECT CodCategoria FROM Categoria WHERE NomeCategoria LIKE 'Eletrônicos')
